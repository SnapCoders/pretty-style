...
public class UpdateProduct implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, Exception {
		try {
			...
			int idProduct = Integer.parseInt(m.getParameter("idProduct"));			
			String idsDeleteStr = m.getParameter("idsDelete"); 
			...
			Product product = new Product(idProduct,name, description, pPrice, idMark, user.getProvider().getId(), categories);
			
			ProductService cs = new ProductService();
			ProductPhotoService pps = new ProductPhotoService();
		
			cs.update(product);
			if(idsDeleteStr.length() > 0) {
				pps.deletePhotos(idsDeleteStr);
			}
			...
			while (files.hasMoreElements()) {
				...
				if (fileNameProp.startsWith("file-")) {
					...
					ProductPhoto productPhoto = new ProductPhoto(fileName, url, product.getId());
